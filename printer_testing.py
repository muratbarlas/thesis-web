...
# Open serial port, change 'COM3' to your Arduino's serial port
ser = serial.Serial(port, 9600, timeout=1)

try:
    while True:
        command = input("Enter '1' to send command to Arduino: ")
        if command == '1':
            ser.write(b'1')  # Send '1' as bytes to Arduino
            time.sleep(0.1)  # Give some time for Arduino to process
        else:
            print("Invalid input. Enter '1'.")

except KeyboardInterrupt:
    ser.close()  # Close serial port when script is interrupted